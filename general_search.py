...
def search(problem, function):
    ...
    while True:
        ...
        closed.append(node.STATE)   # we will no longer visit this state
        if function is 1:
            print "The best state to expand with a g(n) = %d and h(n) = %d is..." % (node.DEPTH, 0)
        elif function is 2:
            print "The best state to expand with a g(n) = %d and h(n) = %d is..." % (node.DEPTH, node.MISPLACED)
        else:
            print "The best state to expand with a g(n) = %d and h(n) = %d is..." % (node.DEPTH, node.MANHATTAN)
